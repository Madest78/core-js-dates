...
function dateToTimestamp(date) {
  const myTimesTamp = new Date(date);
...
function getTime(date) {
  ...
  const timeArrey = [hours, minutes, seconds];
  const dateArr = timeArrey.map((num) => `${num < 10 ? `0${num}` : num}`);
  const onlyTime = dateArr.join(':');
  return onlyTime;
}
...
function getDayName(date) {
  ...
  const myFavoriteDay = dateOk.getUTCDay();
  ...
  return daysOfWeek[myFavoriteDay];
}
...
function getNextFriday(date) {
  const randomData = new Date(date);
  const randomDay = randomData.getUTCDay();
  const friday = 5;

  let daysToAdd = friday - randomDay;
  if (daysToAdd <= 0) {
    daysToAdd += 7;
  }
  randomData.setUTCDate(randomData.getUTCDate() + daysToAdd);
  return randomData;
}
...
function getCountDaysOnPeriod(dateStart, dateEnd) {
  const starsData = new Date(dateStart);
  const endData = new Date(dateEnd);

  const difference = endData.getTime() - starsData.getTime();
...
function isDateInPeriod(date, period) {
  ...
  return givenDate >= new Date(start) && givenDate <= new Date(end);
}
...
function formatDate(date) {
  ...
  const localOffsetHours = -4; // Georgia
  currentDate.setHours(currentDate.getHours() + localOffsetHours);
  ...
  return `${month}/${day}/${year}, ${formattedHour}:${padZero(minute)}:${padZero(second)} ${period}`;
}
